...
public class Oauth2Realm extends AuthorizingRealm {
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken token) throws AuthenticationException {
        ...
        UserToken userToken = shiroService.getByToken(accessToken);
        //token失效
        if (ObjectUtils.isEmpty(userToken) || userToken.getExpireDate().getTime() < System.currentTimeMillis()) {
            throw new IncorrectCredentialsException(MessageUtils.getMessage(ErrorCodeMeta.TOKEN_INVALID));
        }

        //查询用户信息
        User user = shiroService.getUser(userToken.getUserId());
        ...
        //获取用户对应的部门数据权限
        List<String> deptIdList = shiroService.getDataScopeList(signatureUser.getId());
